...
# 1) Install plugins listed in .vagrantplugins
...
# 2) Prompt for RHSM details (activation key path recommended)
echo
...
# 3) Create/backup ~/.vagrant.d/Vagrantfile from template
...
Vagrant.configure("2") do |config|
  # Keep Vagrant's default key; avoids regenerating keys per VM
  config.ssh.insert_key = false

  # Disable the default synced /vagrant folder (less noise; avoids RHEL8 quirks)
  ...
  # Optional: if vagrant-vbguest is installed, don't auto-update guest additions
